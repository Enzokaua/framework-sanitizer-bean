...
import static org.framework.core.SanitizerUtils.*;

class SanitizerValidationRules {
    ...
    public static void validateDatePattern(Field field, Object target, List<String> sanitizerForkingReport, String... exceptionMessage) {
        try {
            field.setAccessible(true);
            Object attributeValue = field.get(target);
            DATE_FORMAT.parse(attributeValue.toString());
            sanitizerForkingReport.add(exceptionPersonalize(field, exceptionMessage));
        } catch (IllegalAccessException e) {
            sanitizerForkingReport.add(ILLEGAL_ACCESS_FIELD_EXCEPTION + field.getName());
        } catch (Exception f) {
            sanitizerForkingReport.add(DATE_FORMAT_VALIDATION_EXCEPTION + field.getName());
        }
    }
    ...
    public static void validateDatePatternThrows(Field field, Object target, String... exceptionMessage) {
        try {
            field.setAccessible(true);
            Object attributeValue = field.get(target);
            DATE_FORMAT.parse(attributeValue.toString());
            ...
            throw new SanitizerValidationException(DATE_FORMAT_VALIDATION_EXCEPTION + field.getName());
        }
    }
